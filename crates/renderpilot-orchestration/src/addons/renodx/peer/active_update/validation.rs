...
use super::model::{RenoDxActiveUpdateHostInput, RenoDxActiveUpdateInput};
...
pub(super) fn validate_input<'a>(
    input: &'a RenoDxActiveUpdateInput<'a>,
    ...
    if let Some(host) = host.as_ref() {
        record::ensure_non_host_claims_unchanged(before, after, host.parts().0)?;
    } else {
    ...
    Ok(ValidatedUpdate {
        ...
        addon_before,
        addon_before_bytes,
        addon_bytes: addon_bytes.clone(),
        host: validated_host,
        has_physical_change: addon_changed || host_changed,
    })
}
...
fn validate_host(
    ...
    let (path, snapshot, bytes) = host.parts();
    paths::require_under_roots(path, canonical_game_root, None)?;
    if !path
        .file_name()

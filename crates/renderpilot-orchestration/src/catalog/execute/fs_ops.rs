//! Filesystem mutations for a resolved catalog transition and its revert.
//!
//! The typed transition boundary builds [`CoordinatedFilePlan`] steps and runs
//! them through [`execute_file_plans`](crate::coordinated_files::execute_file_plans):
//!
//! - path-source installs use `OverlayFromPath` (no full-DLL memory load);
//! - baseline archive+remove uses `CreateVerifiedArchiveAndRemove`;

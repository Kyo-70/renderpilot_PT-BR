...
```powershell
cargo fmt --all -- --check
cargo clippy --profile ci --workspace --all-targets --all-features --locked -- -D warnings
cargo nextest run --cargo-profile ci --profile ci --workspace --all-features --locked
cargo test --profile ci --workspace --all-features --locked --doc --no-fail-fast
$env:RUSTDOCFLAGS = "-D warnings"
cargo doc --profile ci --workspace --all-features --no-deps --locked
```

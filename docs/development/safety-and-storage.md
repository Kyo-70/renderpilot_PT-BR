...
The current release contract targets schema v19. Its v16→v17 step replaces weak global scan caches with owner-scoped file observations and typed, fail-closed scan authority; its v17→v18 step adds the singleton shared-Vulkan durable mutation fence; its v18→v19 step adds the typed OptiScaler lifecycle state and neutral proxy-topology aggregates. Schema changes must update the shared release contract, migration steps, physical contract, repository behavior, and tests together.

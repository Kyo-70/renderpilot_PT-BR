...
test('portable runtime release contract accepts exact JSON integer fields', () => {
  ...
  assert.equal(contract.currentSchema, 20);
});
...
test('portable runtime release contract rejects non-lexical, overflow, and wire-shape variants', () => {
  const invalid = [
    ...
    VALID.replace('"currentSchema": 20', '"currentSchema": 21'),
    VALID.replace('"minimumPortableSchema": 4', '"minimumPortableSchema": "4"'),
    VALID.replace('"minimumPortableSchema": 4,', ''),
    VALID.replace('"currentSchema": 20', '"currentSchema": 20, "unknown": 1'),
    VALID.replace('"currentSchema": 20', '"currentSchema": 19, "currentSchema": 20'),
    VALID.replace('"currentSchema": 20', '"\\u0063urrentSchema": 19, "currentSchema": 20'),
    VALID.replace('"currentSchema": 20', '"CurrentSchema": 20'),
  ];

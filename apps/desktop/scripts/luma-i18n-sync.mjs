...
function fail(message) {
  throw new Error(`Luma i18n sync failed: ${message}`);
}
...
export function projectLumaI18nSource(producer) {
  ...
  const messages = producer.messages.map((message, index) => {
    ...
    if (JSON.stringify(localeKeys) !== JSON.stringify(LUMA_LOCALES.toSorted())) {
...
export const lumaOverrides = defineLocalizedCatalog<'${locale}', LumaSourceCatalog>()(${JSON.stringify(messages, null, 2)});
`;
}

export async function createLumaI18nOutputs(producer) {
  const { contract, localized } = projectLumaI18nSource(producer);
...
export async function checkLumaI18nOutputs(outputs) {
  const stale = [];
...
export async function writeLumaI18nOutputs(outputs) {

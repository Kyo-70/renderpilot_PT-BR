...
export const OPTISCALER_LOCALES = Object.freeze([
  'de',
  'es',
  'fr',
  'ja',
...
const OVERRIDES_ROOT = path.join(APP_ROOT, 'ui/src/shared/i18n/messages/overrides/optiscaler');
...
function fail(message) {
  throw new Error(`OptiScaler i18n sync failed: ${message}`);
}
...
export function projectOptiscalerI18nSource(producer) {
  if (producer?.schema_version !== 1 || !Array.isArray(producer.messages)) {
    fail('producer message contract has an unsupported shape');
  }

  const seenIds = new Set();
  const localized = Object.fromEntries(OPTISCALER_LOCALES.map((locale) => [locale, {}]));
  const messages = producer.messages.map((message, index) => {
    ...
    if (JSON.stringify(localeKeys) !== JSON.stringify(OPTISCALER_LOCALES.toSorted())) {
      fail(`message ${id} does not have exact locale coverage`);
    }
    for (const locale of OPTISCALER_LOCALES) {
    ...
    return {
      ...
      kind: nonEmptyString(message?.guidance_kind, `message ${id} guidance_kind`),
  ...
  try {
    validateOptiscalerContract(contract);
  } catch (cause) {
    fail(
...
export const optiscalerOverrides = defineLocalizedCatalog<'${locale}', OptiScalerSourceCatalog>()(${JSON.stringify(messages, null, 2)});
`;
}

export async function createOptiscalerI18nOutputs(producer) {
  const { contract, localized } = projectOptiscalerI18nSource(producer);
...
export async function checkOptiscalerI18nOutputs(outputs) {
  const stale = [];
...
export async function writeOptiscalerI18nOutputs(outputs) {
